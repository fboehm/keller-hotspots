...
readRDS("Chr7hot_nonlocal.rds") -> hotspot
readRDS("Chr7hot_local.rds") -> local

# load chr2 allele probabilities
readRDS("Chr7_aprobs.rds") -> geno # genoprobs_chr2.rds is on SQUID

# load kinship matrix (LOCO, ie, for chromosome 2, ie, doesn't use chr2 data)
readRDS("Chr7_kinship.rds") -> kinship
...
map <- readRDS("map.rds")
s_out <- tidy_scan_pvl(s_out, pmap = map$`7`) # changes for each hotspot
