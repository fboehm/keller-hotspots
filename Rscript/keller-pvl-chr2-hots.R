...
nhot <- 147 # number of trans hotspot traits for this hotspot
...
readRDS("Chr2hot_nonlocal.rds") -> hotspot

# load chr2 allele probabilities
readRDS("Chr2_aprobs.rds") -> geno # genoprobs_chr2.rds is on SQUID

# load kinship matrix (LOCO, ie, for chromosome 2, ie, doesn't use chr2 data)
readRDS("Chr2_kinship.rds") -> kinship
...
map <- readRDS("map.rds")
s_out <- tidy_scan_pvl(s_out, pmap = map$`2`) # changes for each hotspot
